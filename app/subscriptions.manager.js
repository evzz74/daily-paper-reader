...
window.SubscriptionsManager = (function () {
        ...
        <div id="arxiv-search-panel-body">
          ...
          <div id="arxiv-search-quick-run-side">
            <div style="display:flex; align-items:center; justify-content:space-between; gap:8px; margin-bottom:8px;">
              <div class="chat-quick-run-title" style="margin:0;">文献管理</div>
            </div>

            <div class="chat-quick-run-info" style="padding:12px; background:#f0f4ff; border-radius:6px; margin-bottom:12px; font-size:13px; color:#333;">
              <strong>📤 用户上传模式</strong><br/>
              现在支持通过右下角的上传按钮添加自己的文献，不再依赖AI自动搜集。
            </div>

            <button id="dpr-open-upload-btn" class="chat-quick-run-item" type="button" style="background:linear-gradient(135deg, #667eea 0%, #764ba2 100%); color:white; border:none;">
              📤 打开上传面板
            </button>

            <div class="chat-quick-run-divider" aria-hidden="true"></div>

            <div class="chat-quick-run-title">我的上传</div>
            <div id="dpr-user-uploads-list" class="dpr-user-uploads-list" style="max-height:200px; overflow-y:auto;">
  ...
  const bindBaseEvents = () => {
    ...
    // 绑定打开上传面板按钮
    const openUploadBtn = document.getElementById('dpr-open-upload-btn');
    if (openUploadBtn && !openUploadBtn._bound) {
      openUploadBtn._bound = true;
      openUploadBtn.addEventListener('click', () => {
        if (window.DPRUserUpload && typeof window.DPRUserUpload.open === 'function') {
          window.DPRUserUpload.open();
        } else {
          alert('上传模块尚未加载，请刷新页面后重试。');
        }
      });
    }

    // 加载并显示用户上传的文献列表
    const refreshUserUploadsList = () => {
      const listEl = document.getElementById('dpr-user-uploads-list');
      if (!listEl) return;

      if (window.DPRUserUpload && typeof window.DPRUserUpload.getUploads === 'function') {
        const uploads = window.DPRUserUpload.getUploads();
        if (uploads.length === 0) {
          listEl.innerHTML = `
            <div style="padding:12px; color:#999; font-size:13px; text-align:center;">
              暂无上传的文献
    ...
    // 初始加载上传列表
    refreshUserUploadsList();

    // 监听上传完成事件，刷新列表
    document.addEventListener('dpr-user-upload-complete', refreshUserUploadsList);
